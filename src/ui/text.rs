...
/// Keeps both ends of `text` within `max_width` columns. A path is what tells
/// the user which machine and directory they are in, so the middle is the part
/// that can go.
...
mod tests {
    ...
    fn middle_elide_uses_display_width() {
        let text = middle_elide("/home/me/projects/herdr/src/client/shell/state.rs", 24);
