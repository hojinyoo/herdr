...
use crate::protocol::file_transfer::{FILE_TRANSFER_CHUNK_SIZE, MAX_FILE_TRANSFER_SIZE};

/// Why a transfer was refused. `Display` output travels on the wire in
/// the transfer's end control and is shown verbatim in the progress popup, so
/// it has to read as a user-facing sentence.
#[derive(Debug)]
...
fn is_windows_reserved_name(name: &str) -> bool {
    ...
    // `COM¹` is a device even though it is not ASCII. Device names are
    // case-insensitive, so `Com¹` is the same device: fold the prefix before
    // stripping it rather than listing the spellings.
    if stem.len() < 4 {
        return false;
    }
...
mod tests {
    ...
    fn checked_name_rejects_everything_that_escapes_the_destination() {
        for name in [
            ...
            "COM¹",
            "lpt².txt",
            // Device names are case-insensitive, so the mixed spellings of the
            // superscript aliases are the same devices.
            "Com¹",
            "cOm².txt",
            "Lpt³",
        ] {
