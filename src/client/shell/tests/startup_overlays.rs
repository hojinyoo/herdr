...
fn update_ready_menu_opens_client_owned_release_notes_and_dismisses_by_version() {
    ...
    let update_index = global_menu_index(&state, "update ready");
    let update_row = state.hits.global_menu_rows[update_index].0;
    ...
    state.activate_global_menu_item(update_index, &mut ClientShellInput::default());
...
fn coalesced_release_notes_open_and_scroll_uses_current_geometry() {
    ...
    state.overlay = Some(ClientShellOverlay::GlobalMenu(ClientGlobalMenuOverlay {
        highlighted: global_menu_index(&state, "update ready"),
    }));
