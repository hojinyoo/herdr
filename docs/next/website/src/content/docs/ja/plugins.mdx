...
アクションの `contexts` は、そのアクションをどの右クリックメニューに出すかを
決めます。`workspace` はサイドバーのワークスペースメニューに、`pane` はペイン
メニューに追加し、いずれも Herdr 自身の項目より後ろに並びます。メニューからの
実行では、アクティブなワークスペースではなくクリックされた行の情報が渡され、
`invocation_source = "context_menu"` も付きます。行をクリックしても Herdr の
フォーカスは動かないので、`herdr` コマンドがフォーカス中のペインにフォール
バックしないよう、コンテキストの id を使って操作してください。それ以外の
context 値は解析されますが、まだメニュー項目にはなりません。`contexts` がない
アクションも、キーバインド、リンクハンドラー、`herdr plugin action invoke` から
実行できます。

メニュー項目が出るのは、それを配信できる新しいサーバーに接続していて、かつ
プラグインが有効でアクションがサーバーのプラットフォームに対応している場合
だけです。
