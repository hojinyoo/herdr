...
使用 `--remote` 连接时，Herdr 可以通过自有协议在你的机器和远程面板之间传输单个文件，
并在 Herdr 界面中显示进度和取消操作。在全局菜单中选择 **send file** 或 **receive file**，
或右键点击面板并选择 **Send file...** / **Receive file...**。

**接收**会打开远程目录浏览器：方向键移动，Enter 打开文件夹或选择文件，输入文字进行筛选，
`.` 切换隐藏文件，Esc 取消。浏览器从当前聚焦面板的工作目录开始，并会显示决定接收位置的
配置项名称。

**发送**会询问你本机上的路径。可以直接输入，也可以**把文件拖放到已打开的弹窗上** —— 路径会
自动填入，包含空格的文件名同样有效。

- **发送** 读取本地机器上的路径，并写入当前聚焦面板的工作目录。
- **接收** 按当前聚焦面板工作目录的相对路径读取远程主机上的文件，并写入
  `remote.file_transfer_dir`（默认为 `~/Downloads`）。

Herdr 不会覆盖已存在的文件。如果目标位置已存在同名文件，新文件会以编号后缀保存
（`notes.txt` -> `notes-1.txt`），并在弹窗中显示实际写入的文件名。按 Esc 取消正在进行的
传输，写入一半的文件会被删除。

同一时间只能运行一个传输，文件大小上限为 256 MiB，暂不支持目录。超过 60 秒没有任何进展的
传输会被放弃，因此不会长期占用传输名额。

:::caution[信任]
发送由 Herdr 服务器驱动：服务器会向你的客户端索取你所输入路径的文件。因此被入侵的远程
服务器可以索取你并未选择的路径，而客户端会将其发送出去。在此功能出现之前，服务器完全
无法读取你本地的文件系统。请仅对你愿意托付本地文件的服务器使用 `--remote`。
:::传输会在每个 256 KiB
分块得到确认后才发送下一个，因此吞吐量受往返延迟限制：256 MiB 的文件在快速链路上大约需要
20 秒，在远距离链路上会明显更久。传输不支持断点续传，连接中断后需要重新开始。
