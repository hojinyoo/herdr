...
`.` 切换隐藏文件，Esc 取消。浏览器从当前聚焦面板的工作目录开始，并会显示接收文件的目标
目录。
...
- **接收** 读取远程主机上的路径，并写入 `remote.file_transfer_dir`（默认为 `~/Downloads`）。
...
传输会被放弃，因此不会长期占用传输名额。传输会在每个 256 KiB 分块得到确认后才发送下一个，
因此吞吐量受往返延迟限制：256 MiB 的文件在快速链路上大约需要 20 秒，在远距离链路上会明显
更久。传输不支持断点续传，连接中断后需要重新开始。

只有客户端能发起传输，且只针对你选定的文件，因此远程服务器无法主动读取你本地的文件系统。
