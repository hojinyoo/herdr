...
动作的 `contexts` 决定它出现在哪些右键菜单里。`workspace` 把动作加入侧边栏的
工作区菜单,`pane` 把它加入窗格菜单,两者都排在 Herdr 自身的条目之后。从菜单
调用时,传入的是被点击的那一行而不是当前活动的工作区,并带上
`invocation_source = "context_menu"`。点击某一行不会移动 Herdr 自身的焦点,
所以请使用上下文里的 id 来操作,不要让 `herdr` 命令回退到当前聚焦的窗格。其他
context 值可以解析,但目前还不会生成菜单条目。没有 `contexts` 的动作仍然可以
通过快捷键、链接处理器和 `herdr plugin action invoke` 调用。

只有连接到能够下发菜单条目的新版服务器,并且插件已启用、动作支持服务器所在的
平台时,菜单条目才会出现。
