...
the pane menu, always after Herdr's own entries. A menu invocation describes the row that was
clicked rather than the active workspace, and arrives with
...
back to the focused pane. Other context values parse but do not add
a menu entry yet. Actions without `contexts` stay reachable through keybindings,
link handlers, and `herdr plugin action invoke`.
