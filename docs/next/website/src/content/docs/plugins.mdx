...
the pane menu, always after Herdr's own entries. A menu invocation runs against
the row that was clicked rather than the active workspace, and arrives with
`invocation_source = "context_menu"`. Other context values parse but do not add
