...
An action's `contexts` list decides which right-click menus offer it.
`workspace` puts the action in the sidebar workspace menu and `pane` puts it in
the pane menu, always after Herdr's own entries. The invocation describes the
row that was clicked rather than the active workspace, and arrives with
`invocation_source = "context_menu"`. Clicking a row does not move Herdr's own
focus, so act on the ids in the context rather than letting a `herdr` call fall
back to the focused pane. Other context values parse but do not add a menu entry
yet. Actions without `contexts` stay reachable through keybindings, link
handlers, and `herdr plugin action invoke`.

A menu entry is offered only by a server new enough to publish it, and only when
the plugin is enabled and the action supports the server's platform.
