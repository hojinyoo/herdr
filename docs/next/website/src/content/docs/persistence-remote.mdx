...
directory received files will land in.

**Sending** asks for a path on your own machine. You can type it, or simply
**drag the file onto the open popup** and the path is filled in for you, including
names with spaces.

- **Send** reads the path on your local machine and writes it into the focused
  pane's working directory.
- **Receive** reads the path on the remote host and writes it into
  `remote.file_transfer_dir` (`~/Downloads` by default).
...
not supported yet. A transfer that makes no progress for 60 seconds is abandoned
so it cannot hold the slot indefinitely. Both sides acknowledge each 256 KiB
chunk before the next one goes out, so throughput is bounded by round-trip
latency: expect roughly 20s for a 256 MiB file on a fast link and appreciably
longer on a distant one. There is no resume, so a dropped connection restarts
the transfer.

Only your client can start a transfer, and only for a file you picked, so a
remote server cannot reach into your local filesystem on its own.
