...
Herdr never overwrites: if a file of that name already exists at the
destination, the transfer is refused and the popup says so. Press Esc to cancel
a running transfer; a partially written file is removed.

One transfer runs at a time, files are limited to 16 MiB, and directories are
not supported yet.
